...
- **risk:** HIGH
  - **3 FERPA cross-tenant read leaks closed** (submissions single-GET [CRITICAL — any authed non-student could read any submission by id; confirmed by two independent audit agents], adaptive GET, assignment-diff GET). Fixes mirror the ownership pattern already in `files/route.ts` and the routes' own list branches.
  - **Auth/privilege:** district/access mass-assignment, verify-otp TOCTOU, district/students cross-district reassign.
  - **Schema change ships here** — additive/nullable, `prisma db push` runs in the build script; `prisma validate` + `generate` verified locally.
- **review:** ⚠️ partial — verified by direct code re-read of every changed file + full `tsc --noEmit` (0 `src` errors, down from 21). NOT verified by a full `next build`/runtime (this env has no DB, so the build's `prisma db push` + `next build` path was not exercised). A REVIEWER/TESTER on a DB-connected env should: `npm run build`, then per-role smoke the 3 FERPA GETs (403 for a non-owning teacher/parent/admin; 200 for the legitimate owner) and confirm demo study-groups/marketplace/messages no longer 500/403.
...
### 3dcd76a — `v17.1.0 — Update 6.1: OWNER nav wiring, per-product subscriptions, entitlement gates, UX + security cleanup`
- **scope:** feat(ux/auth/security/billing) — v17.1.0
- **files:** ~39 · ~34 modified + 5 new. New: `src/lib/entitlement.ts` (shared per-product / per-bundle ownership resolver — returns `402` with `checkoutUrl` for unentitled callers), `src/components/AnonShell.tsx` (shared chrome wrapper for unauthed non-landing pages — applied to `/pricing`, `/onboard`, `/register`, `/demo`), `src/app/api/contact/route.ts` (Resend-backed contact form submit with rate limit + audit log + reply-to handling), `src/app/api/products/[productId]/cancel/route.ts` (mirrors the bundle cancel shape — ownership-checked, master-demo synthetic success), `src/app/api/assignments/[id]/route.ts` (PUT + DELETE handlers so teachers can save assignment edits — the page surface for editing already existed). Modified: `src/middleware.ts` (OWNER role bucket added to the role-gate table; `/api/contact` added to `PUBLIC_API_PATHS`), `src/lib/auth.ts` (NextAuth `redirect` callback added with same-origin guard against open-redirect via `callbackUrl`; `DEMO_EMAIL_ROLES` TEACHER entry for the master demo removed; master-demo gates on STUDENT pages restored), `src/lib/payments.ts` + `src/app/api/payments/route.ts` (`TIER_PRICES` constant — Starter $2 / Growth $4 / Enterprise $8 per student/year; non-OWNER upgrade calls constrained to these values, custom prices return 403 with audit-log entry), `src/app/api/study/generate/route.ts` + `src/app/api/practice/generate/route.ts` + `src/app/api/products/generate/route.ts` (entitlement gate via `src/lib/entitlement.ts`), `src/app/api/auth/forgot-password/route.ts` (Resend send was commented out — re-enabled, wired to the new `passwordResetEmail` template; generic success-message-regardless-of-account preserved to avoid user enumeration), `src/lib/email-templates.ts` (+`passwordResetEmail`), `src/components/layout/DashboardLayout.tsx` (OWNER sidebar nav entries — Finances + Prices; OWNER added to mobile nav + role-color/label map + master-demo role switcher; sidebar logo now a clickable `<Link>` to `/`), `src/components/AuthAwareCTA.tsx` (`dashboardHrefFor` routes OWNER role to `/owner` BEFORE the `isMasterDemo` branch), `src/components/dashboard/MySubscriptionsCard.tsx` (mixed bundle + product counts), `src/components/products/MarkdownToolPage.tsx` (402 entitlement errors caught — friendly toast + redirect to the product's checkout, instead of generic error), `src/app/account/subscriptions/page.tsx` (new "Single-product subscriptions" section with per-product cancel), `src/app/api/products/subscriptions/route.ts` (returns BOTH `bundleSubscriptions` AND `productSubscriptions`; the existing `subscriptions` key preserved as a deprecated alias), `src/app/teacher/lesson-planner/page.tsx` (every `s.value` replaced with `s.id` — `SUBJECTS` uses `id`/`label`/`emoji`, so `s.value` was always `undefined`), `src/app/admin/employees/page.tsx` (Send Message / Edit Profile buttons no longer stub no-ops), `src/app/(legal)/contact/page.tsx` (real submit replacing the v17.0 mailto fallback), `src/app/(legal)/accessibility/page.tsx` (`.edu` → `.co`; placeholder phone removed), `src/app/help/page.tsx` (FAQ matches real Family pricing — $9/mo or $7/mo annual + 14-day trial, no longer says "free for 5 children"; quick-links grid collapsed 4-cols-3-cells → 3 cols; AI Tutor quick-link → "Try a tool" → `/products`; two new product-FAQ entries covering the 13 individuals + 4 bundles), `src/components/landing/LandingPage.tsx` (CTAs unified toward "Start free trial" → `/onboard`; JSON-LD `foundingDate` corrected to 2025; Family Offer price corrected to 9; footer Products de-duplicated; Demo moved to Company column; fuchsia Products link recolored to primary; skip-to-content link added; footer cross-links to `/pricing` and `/roadmap`), `src/app/(auth)/{demo,login,onboard,pricing,register}/page.tsx` (AnonShell adoption; login default-fallback `/student/dashboard` → `/`; OWNER 2FA Resend-OTP + 5-min countdown; `/demo` catches MFA_REQUIRED for OWNER-promoted demo accounts and redirects to `/login` instead of erroring silently), `src/app/products/page.tsx` (Owned badges fix — was reading `data.bundles` from an API that returns `data.subscriptions`; `id="bundles"` added so `/products#bundles` anchor scrolls), `src/app/owner/page.tsx` (authenticated OWNER landing — `/` now redirects OWNER to `/owner`), `src/app/api/auth/verify-otp/route.ts` (notifications-page enumeration: same generic error for wrong-code AND missing-challenge paths), `package.json` (16.9.0 → 17.1.0; note: was never bumped to 17.0.0 in the prior release — corrected here), `CHANGELOG.md`, `CODE-REVIEW.md`.
- **risk:** MEDIUM
  - **Broad UX cleanup surface.** ~34 modified files across landing, auth, dashboards, account, owner nav, contact, accessibility, help, lesson planner, admin employees. The change is wide but shallow — most touches are small edits to extend or harden patterns already established in 6.0.
  - **1 CRITICAL recovery-flow fix.** Forgot-password's Resend send was commented out in the route handler, so the entire password-recovery flow returned a generic success message but never delivered an email. The visible-from-the-outside symptom was indistinguishable from a working flow that just hadn't been triggered yet — users with locked-out accounts could not recover them in production. Re-enabled and wired to a new `passwordResetEmail` template. The generic-success response shape (regardless of whether the email matches a real account) is preserved to prevent user enumeration.
  - **2 HIGH security fixes.** (a) `/api/payments` upgrade action: non-OWNER callers (notably ADMIN) could submit a custom `pricePerYear` and the route would write it. Now constrained to a server-side `TIER_PRICES` constant; custom prices from non-OWNER return 403 with an audit-log entry. OWNER retains the ability to override via the immutable `PriceOverride` store from 6.0 (which goes through `/api/owner/prices`, not `/api/payments`). (b) `/api/study/generate` and `/api/practice/generate` now enforce per-product entitlement via `src/lib/entitlement.ts`. Previously any authenticated user could call them freely, bypassing the per-product paywall the catalog and checkout pages advertise. The third generate route, `/api/products/generate`, was also wired through the same helper for consistency. Master demo short-circuits to `entitled: true` so the demo-mode contract holds.
  - **MED open-redirect hardening.** NextAuth `redirect` callback added with a same-origin guard — off-origin `callbackUrl` values now fall back to the role's dashboard.
  - **No schema changes.** Schema from 6.0 covers everything in 6.1. The optional `CONTACT_EMAIL_TO` env var is the only new env, defaulting to `contact@limud.co`. The existing `RESEND_API_KEY` (added in 6.0 for OWNER 2FA) is now used by the forgot-password flow too.
- **review:** APPROVED WITH NOTES (pending REVIEWER pass — flip to ✅ reviewed once REVIEWER signs off). REVIEWER pass must verify:
  1. **OWNER nav wiring.** Log in as OWNER → sidebar shows Finances + Prices; mobile nav same; role color/label render correctly; clicking sidebar logo navigates to `/`. Log in as STUDENT/TEACHER/PARENT/ADMIN → no OWNER entries surface. Master-demo role switcher shows 5 buttons including OWNER. Edge middleware blocks non-OWNER access to `/owner/**` with a 403 from the role gate (NOT from the layout fallback).
  2. **Forgot-password (CRITICAL).** Submit `/forgot-password` with a real account email → Resend log shows the send fired, the recipient receives the reset email with a working link. Submit with a non-existent email → SAME generic success message returned (no enumeration), but no email is delivered.
  3. **Custom pricePerYear (HIGH).** As ADMIN, POST `/api/payments` with action=upgrade and a custom `pricePerYear` not in `TIER_PRICES` → expect 403 + audit-log entry. As OWNER, same call via `/api/owner/prices` → expect success (immutable `PriceOverride` write). As non-OWNER with a value matching `TIER_PRICES.Growth` ($4) → expect success.
  4. **Entitlement (HIGH).** As an unentitled authenticated user, POST `/api/study/generate` / `/api/practice/generate` / `/api/products/generate` → expect 402 with `checkoutUrl` payload. Purchase the product → retry → expect 200. As master demo → expect 200 on every call (demo entitlement bypass). Frontend MarkdownToolPage catches the 402 and redirects to checkout with a friendly toast, NOT a generic error.
  5. **NextAuth open-redirect (MED).** Submit a login with `callbackUrl=https://evil.example/path` → expect the redirect callback to drop the off-origin URL and fall back to the role's dashboard.
  6. **Per-product subscription management.** Purchase two products + one bundle. Visit `/account/subscriptions` → bundle section + Single-product section both render. Cancel a product → row status flips, dashboard widget mixed-count updates. `MySubscriptionsCard` on STUDENT/TEACHER/PARENT/ADMIN dashboards shows the correct mixed bundle + product count.
  7. **Lesson planner subject chip.** Visit `/teacher/lesson-planner` → SUBJECTS render with emoji + label, clicking a subject chip highlights it (previously every chip was `undefined`).
  8. **Contact form.** Submit anonymously → email lands at `contact@limud.co` (or `CONTACT_EMAIL_TO` override), audit log entry exists, reply-to is the submitter's email. Repeat-submit hits the rate limit. Anonymous submit reaches the route (verify `/api/contact` is in PUBLIC_API_PATHS — anonymous traffic was getting bounced before).
  9. **Master demo on student surfaces.** Master demo → `/student/dashboard` and `/student/survey` both load (post-SEC-3 they were 403ing because the hard STUDENT gates were kicking the master demo out). The gates now check for master demo explicitly.
  10. **Dead-end UX sweep.** Authenticated OWNER on `/` → redirects to `/owner`. Login fallback for an unknown role → `/` not `/student/dashboard`. `/products` "Owned" badges render after a bundle purchase. `/products#bundles` anchor scrolls. Help quick-link "Try a tool" → `/products`. Admin Send Message / Edit Profile buttons in `/admin/employees` route somewhere real.
- **demo-mode:** PASS — preserved write-gates; master-demo can now reach student surfaces for visual demo.
  - Entitlement helper short-circuits to `entitled: true` for `isMasterDemo` — every generate route still works for the demo walkthrough.
  - Per-product cancel returns a synthetic success record without writing for master demo (mirrors the bundle cancel from 5.8 and the per-product purchase from 6.0).
  - Contact route does NOT short-circuit on master demo — the form is a real channel to the inbox, not a demo prop; the rate limiter prevents abuse from the demo surface.
  - Forgot-password does NOT short-circuit on master demo — same reason; the prior behavior was already broken so nothing regresses.
  - `DEMO_EMAIL_ROLES` no longer maps the master demo email to `TEACHER` — the master demo now flows through the OWNER role assignment from 6.0 and lands on `/owner` by default.
  - Master demo's access to `/student/dashboard` and `/student/survey` is restored. The hard STUDENT role gates were enforcing strict role equality after SEC-3 removed the universal bypass; the gates now special-case master demo for visibility (no DB writes, no role escalation — same pattern as the OWNER demo path).
- **tests:** TESTER pass pending. No automated tests added — Limud has no test suite for entitlement, contact, or forgot-password yet (matches prior releases). Manual smoke per the 10-item REVIEWER checklist above is the test plan; the TESTER will also run `npm run build` to catch any TS regressions across the ~34 files touched. The build script does NOT need to run `prisma db push` for this release (schema unchanged from 6.0).
- **notes:**
  - **Schema: no changes.** `npx prisma db push` is NOT required. The 6.0 schema covers everything in 6.1.
  - **SEC-1 from 6.0 is still pending.** Rotate `NEXTAUTH_SECRET`, `RESEND_API_KEY`, `CRON_SECRET`, `PII_ENCRYPTION_KEY` in the Render dashboard. Until that happens, the values committed to `.env.render` must continue to be treated as compromised. This release adds a second consumer of `RESEND_API_KEY` (forgot-password) on top of the OWNER 2FA flow from 6.0, increasing the blast radius of a compromised key — rotate sooner rather than later.
  - **Production redeploy required.** OWNER nav wiring, per-product subscription surfaces, entitlement gates, contact route, and the forgot-password fix all need a fresh deploy. The forgot-password fix in particular is a CRITICAL user-facing repair — every day it sits unmerged is another day with a broken recovery flow.
  - **`AuthAwareCTA` copy unification flagged as v17.2 follow-up.** CODER 4 noted that some CTA labels are still owned by `AuthAwareCTA` and didn't migrate to the unified "Start free trial" → `/onboard` copy in this release. The unification is intentionally partial; the rest of the labels move in v17.2 to keep this release's diff scope reviewable.
  - **`package.json` version was stuck at 16.9.0** at the start of this release (the v17.0.0 release shipped CHANGELOG + CODE-REVIEW updates but missed the package.json bump). Corrected here to 17.1.0. The COO should note that the prior release's tag-vs-package mismatch could complicate any post-incident `git log`/`git tag` reconciliation.
  - **Commit SHA is `(pending)`** — backfill once the v17.1.0 commit lands on `main`. Until that backfill, the SHA placeholder is the indicator that this entry is for a release that hasn't been recorded in `git log` yet.
  - **Review state is APPROVED WITH NOTES (pending REVIEWER pass)** — flip to ✅ reviewed once REVIEWER signs off on the 10-item checklist. If REVIEWER comes back with findings, the entry flips to ⚠️ partial pending DEBUGGER + re-verification.
