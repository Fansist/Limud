...
/**
 * STUDENT subtree error boundary.
 *
 * v17.5: catches any uncaught render/data error inside `/student/**` so the
 * STUDENT lands on a branded fallback instead of Next.js's default error
...
export default function StudentErrorPage({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    ...
    console.error('[Limud][student] route error', error?.digest, error?.message);
  }, [error]);
    ...
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Something on the student page hit an unexpected error. Try again, or head back to your dashboard.
        </p>
        {error?.digest && (
