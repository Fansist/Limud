...
/**
 * TEACHER subtree error boundary.
 *
 * v17.5: catches any uncaught render/data error inside `/teacher/**` so the
 * TEACHER lands on a branded fallback instead of Next.js's default error
...
export default function TeacherErrorPage({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    // Log error digest to console so operators can correlate against
    // Render logs. Don't show the raw stack to end users.
    console.error('[Limud][teacher] route error', error?.digest, error?.message);
  }, [error]);
    ...
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          The teacher console hit an unexpected error. Try again, or head back to your dashboard.
        </p>
        {error?.digest && (
