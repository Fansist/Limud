...
export const GET = secureApiHandler(
  async (req, user) => {
    ...
    const compliance = {
      ...
      security: {
        passwordPolicy: `NIST SP 800-63B (min ${SECURITY_CONFIG.password.minLength} chars)`,
        bruteForceProtection: `${SECURITY_CONFIG.lockout.maxFailedLogins} attempts → progressive lockout`,
        rateLimiting: `${SECURITY_CONFIG.rateLimits.global} req/min global`,
        encryption: SECURITY_CONFIG.encryption.algorithm,
        sessionPolicy: `${SECURITY_CONFIG.session.maxAgeHours}h max age`,
        securityHeaders: 'HSTS, CSP, X-Frame-Options, X-Content-Type-Options',
        auditRetention: `${Math.round(SECURITY_CONFIG.audit.retentionDays / 365)} years`,
