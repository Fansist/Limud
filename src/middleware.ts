/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  LIMUD v9.3.3 — Next.js Edge Middleware                                  ║
 ...
 * ║                                                                        ║
 * ║  v9.3.3: All secrets embedded — zero env vars required.                  ║
...
export async function middleware(request: NextRequest) {
  ...
  const token = await getToken({
    req: request,
    secret: AUTH_SECRET,
    // v9.3.3: match the plain cookie name set in auth.ts
...
function addSecurityHeaders(response: NextResponse, pathname: string) {
  ...
  response.headers.set('X-Limud-Version', '9.3.3');
