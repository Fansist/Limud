...
function addSecurityHeaders(response: NextResponse, pathname: string) {
  ...
  // v17.4: bumped from the stale '11.0.0' string that had been left behind
  // since the v11 release. Kept as a string literal (rather than reading
  // package.json) because middleware runs on the Next.js edge runtime,
  // which cannot import JSON modules.
  response.headers.set('X-Limud-Version', '17.4.0');
